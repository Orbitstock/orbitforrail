...
@admin.register(CryptoRecovery)
class CryptoRecoveryAdmin(admin.ModelAdmin):
    list_display = ('crypto_type', 'wallet_type', 'wallet_address', 'amount_lost', 'submitted_at')
    search_fields = ('wallet_address', 'crypto_type', 'wallet_type')
    list_filter = ('crypto_type', 'wallet_type', 'submitted_at')
...
class UserAdminForm(forms.ModelForm):
    ...
    class Media:
        ...
        js = ('admin/js/custom.js',)  # Optional, but good placeholder for future

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['is_banned'].widget.attrs.update({'class': 'toggle-switch'})
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = BaseUserAdmin.list_display + (
        'is_banned', 'is_active', 'email', 'username', 'full_name', 'contact_no', 'balance', 'account_status'
    )
    list_filter = BaseUserAdmin.list_filter + ('is_banned', 'is_active')

    # Customize the fieldsets to exclude password, permissions, groups, and user permissions
    fieldsets = (
        (('Personal info'), {
            'fields': ('username', 'first_name', 'last_name', 'email')
        }),
        (('Account Details'), {
            'fields': ('contact_no', 'is_active', 'is_banned')
        }),
        (('Important dates'), {
    ...
    # Remove password, groups, and permissions from the form
    def get_form(self, request, obj=None, **kwargs):
        form = super().get_form(request, obj, **kwargs)
        for field in ['password', 'user_permissions', 'groups']:
